# Contributing to SAE Books — Desktop Client

`saebooks-desktop` is in public alpha (v0.1) alongside the
...
process across all SAE Books repositories.

## Licence

`saebooks-desktop` is licensed under AGPL-3.0, the same licence as the
...
repositories under one signature per contributor.

The CLA text lives in `CLA.md` in the
[saebooks](https://github.com/saebooks/saebooks) repo. In plain
...
`security@saee.com.au`. See [SECURITY.md](./SECURITY.md).
