...
Used for both CREATE (new blank) and EDIT (pre-filled from existing bill).

Constructor:
    BillForm(client, bill_id=None, parent=None)

If ``bill_id`` is provided the form loads the existing bill data via
``GET /api/v1/bills/{id}`` and pre-fills all fields.

Signals:
    bill_saved(str)             — emitted with the saved bill id after success
    cancelled()                 — emitted when Cancel is clicked
    new_supplier_requested()    — emitted when "New Supplier" is clicked (wired later)
"""
...
from saebooks_desktop.services.bill_form import (
    create_bill,
...
class BillForm(QWidget):
    """Create/edit form for a single bill.

    In create mode (``bill_id=None``) the form is blank.
    In edit mode an existing bill is loaded and fields pre-filled.
    """

    bill_saved = Signal(str)
    cancelled = Signal()
    new_supplier_requested = Signal()
    ...
    def __init__(
        ...
        contact_row_layout.addWidget(QLabel("Supplier:"))
        self._contact_combo = QComboBox()
        self._contact_combo.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
        )
        contact_row_layout.addWidget(self._contact_combo)
        self._new_contact_btn = QPushButton("New Supplier")
        self._new_contact_btn.clicked.connect(self.new_supplier_requested)
        ...
        num_ref_layout.addWidget(QLabel("Bill #:"))
        ...
        # Load reference data then optionally the existing bill
        self._load_reference_data()
        if self._bill_id:
            self._load_existing_bill()
        else:
    ...
    def _load_reference_data(self) -> None:
        """Fetch suppliers, accounts, and tax codes from the API."""
        try:
        ...
        try:
            self._accounts = list_expense_accounts(self._client)
        except (ServerOfflineError, Exception):  # noqa: BLE001
            self._accounts = []
        ...
        self._contact_combo.addItem("-- Select Supplier --", userData=None)
    ...
    def _load_existing_bill(self) -> None:
        """Fetch existing bill and pre-fill form fields."""
        assert self._bill_id is not None
        try:
            data = get_bill(self._client, self._bill_id)
        except (ServerOfflineError, Exception) as exc:  # noqa: BLE001
            self._show_banner(f"Could not load bill: {exc}", error=True)
            self._append_blank_line()
            return
    ...
    def _build_payload(self, status: str) -> dict[str, Any] | None:
        ...
        if contact_id is None:
            self._show_banner("Please select a supplier.", error=True)
            return None
    ...
    def _do_save(self, payload: dict[str, Any], *, post_after: bool) -> None:
        ...
        try:
            if self._bill_id is None:
                # Create
                result = create_bill(self._client, payload)
                ...
                status_code, result = update_bill(
                    self._client, self._bill_id, patch_payload, etag
                )
                if status_code == 409:
                    self._show_banner(
                        "Version conflict — another user has modified this bill. "
                        "Please cancel and reload.",
                        error=True,
                    )
                    return
                saved_id = str(result.get("id", self._bill_id))
                saved_version = result.get("version")

            if post_after:
                etag_for_post = saved_version if saved_version is not None else 1
                post_bill(self._client, saved_id, etag_for_post)

        except Exception as exc:  # noqa: BLE001
            self._show_banner(f"Save failed: {exc}", error=True)
            return

        self.bill_saved.emit(saved_id)
